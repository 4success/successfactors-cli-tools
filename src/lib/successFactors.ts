...
export default class SuccessFactors {
  ...
  private async saveUserPhoto(user: UserBasicData, folderToSaveImages: string): Promise<CsvOutput> {
    ...
    try {
      const auth =
        'Basic ' +
        Buffer.from(`${this.credentials.username}@${this.credentials.companyId}:${this.credentials.password}`).toString(
          'base64',
        );
      ...
      const filename = `${folderToSaveImages}/${user.username}.${SuccessFactors.getFileSuffixForContentType(
        image.type,
      )}`;
  ...
  private async getUsersBasicData(): Promise<UserBasicData[]> {
    ...
    do {
      ...
      if (!nextUrl) {
        const req = await odata(oDataConfig)
          .resource('User')
